...
Node* makeNode(Node* parent, int key, int *memory) {
    *memory += (sizeof(Node) + sizeof(node));
...
Node* addNode(Node* cur, int key, int *memory) {
    if(cur->key == key)
        return cur;
    ...
    if(cur->key > key) {
        if(cur->left)
            tmp = addNode(cur->left, key, memory);
        else
            tmp = cur->left = makeNode(cur, key, memory);
    }
    else {
        if(cur->right)
            tmp = addNode(cur->right, key, memory);
        else
            tmp = cur->right = makeNode(cur, key, memory);
    }
...
SplayTree* initSplayTree(int* memory) {
    SplayTree* st = (SplayTree*)malloc(sizeof(SplayTree));
    *memory += sizeof(SplayTree) + sizeof(st);
    st->root = NULL;
...
Node* insertKey(SplayTree* tree, int key, int * memory) {
    if(!tree->root)
        tree->root = makeNode(NULL, key, memory);
     return splay(&tree->root, addNode(tree->root, key, memory));
}
